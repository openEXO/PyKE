...
# identify the season of observation
    ...
    if status == 0:
        try:
            ...
            message = 'ERROR -- KEPTRIM: Cannot find SEASON keyword in file ' + infile + '.'
            status = kepmsg.err(logfile,message,verbose)
...
# construct output light curve extension
    ...
    if status == 0:
        ...
        for i in range(len(cards1)):
            try:
                if cards1[i].key not in hdu1.header.ascardlist().keys():
                    hdu1.header.update(cards1[i].key, cards1[i].value, cards1[i].comment)
                else:
                    hdu1.header.ascardlist()[cards1[i].key].comment = cards1[i].comment
            except:
                pass
...
if '--shell' in sys.argv:
    ...
    keptrim(args.infile,args.maskfile,args.outfile,args.kepid,args.column,args.row,args.imsize,
