...
# input data
    ...
    if status == 0:
        table = instr[1].data
        incards = instr[1].header.ascardlist()
        try:
...
# update HDU1 for output file
    ...
    if status == 0:
        ...
        instr[1].header.ascardlist()['TTYPE'+str(len(instr[1].columns))].comment = 'column title: phase'
        instr[1].header.ascardlist()['TFORM'+str(len(instr[1].columns))].comment = 'data type: float32'
        for i in range(len(incards)):
            if incards[i].key not in instr[1].header.ascardlist().keys():
                instr[1].header.update(incards[i].key, incards[i].value, incards[i].comment)
            else:
                instr[1].header.ascardlist()[incards[i].key].comment = incards[i].comment
...
# write new phased data extension for output file
    ...
    if status == 0 and bindata:
        ...
        instr[-1].header.ascardlist()['TTYPE1'].comment = 'column title: phase'
        instr[-1].header.ascardlist()['TTYPE2'].comment = 'column title: simple aperture photometry'
        instr[-1].header.ascardlist()['TTYPE3'].comment = 'column title: SAP 1-sigma error'
        instr[-1].header.ascardlist()['TTYPE4'].comment = 'column title: pipeline conditioned photometry'
        instr[-1].header.ascardlist()['TTYPE5'].comment = 'column title: PDC 1-sigma error'
        instr[-1].header.ascardlist()['TTYPE6'].comment = 'column title: cotrended basis vector photometry'
        instr[-1].header.ascardlist()['TTYPE7'].comment = 'column title: Detrended aperture photometry'
        instr[-1].header.ascardlist()['TTYPE8'].comment = 'column title: DET 1-sigma error'
        instr[-1].header.ascardlist()['TFORM1'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TFORM2'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TFORM3'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TFORM4'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TFORM5'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TFORM6'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TFORM7'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TFORM8'].comment = 'column type: float32'
        instr[-1].header.ascardlist()['TUNIT2'].comment = 'column units: electrons per second'
        instr[-1].header.ascardlist()['TUNIT3'].comment = 'column units: electrons per second'
        instr[-1].header.ascardlist()['TUNIT4'].comment = 'column units: electrons per second'
        instr[-1].header.ascardlist()['TUNIT5'].comment = 'column units: electrons per second'
        instr[-1].header.ascardlist()['TUNIT6'].comment = 'column units: electrons per second'
