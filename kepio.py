...
import sys, pyfits, tempfile, os, shutil, glob, numpy
...
def readTPF(infile,colname,logfile,verbose):
    ...
    if status == 0:
        try:
            ...
            txt = 'ERROR -- KEPIO.READTPF: No SKYGROUP keyword in ' + infile + '[0]'
            status = kepmsg.err(logfile,txt,verbose)
    if status == 0:
        try:
    ...
    if status == 0:
        try:
            ...
            txt = 'ERROR -- KEPIO.READTPF: No QUARTER keyword in ' + infile + '[0]'
            status = kepmsg.err(logfile,txt,verbose)
    if status == 0:
        try:
            ...
            txt = 'ERROR -- KEPIO.READTPF: No SEASON keyword in ' + infile + '[0]'
            status = kepmsg.err(logfile,txt,verbose)
    if status == 0:
        try:
    ...
    if status == 0:
        try:
            ...
            txt = 'ERROR -- KEPIO.READTPF: No KEPMAG keyword in ' + infile + '[0]'
            status = kepmsg.err(logfile,txt,verbose)
    if status == 0:
        try:
